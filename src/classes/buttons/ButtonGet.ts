...
export default class ButtonGet extends ButtonBase {
  ...
  constructor(scene: Scene, x: number, y: number, handleClick: () => void) {
    const configButtonBase = {
      ...
      x,
      y,
      texture: TextureKeys.BUTTON_GET_PRESED,
    };
  ...
  private pointerDownHandler() {
    ...
    this.setTexture(TextureKeys.BUTTON_GET);
    this.isDown = true;
    this._handleClick();
  }

  private pointerUpHandler() {
    this.setTexture(TextureKeys.BUTTON_GET_PRESED);
    this.isDown = false;
  }
}
